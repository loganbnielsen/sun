let tls_authenticator = lazy (
  ...
  | None ->
    Error
      "obs-tls: no system CA bundle found for HTTPS endpoint; \
       refusing to connect without certificate verification"
)

(* Cache the TLS config (concrete type — no value-restriction issue). *)
let _tls_config = lazy (
  let authenticator =
  ...
  | Ok c -> c
  | Error (`Msg m) -> failwith ("obs-tls: TLS config error: " ^ m)
)

(* Regular function — syntactic value, so the Eio flow row type generalises. *)
let https_wrapper uri raw =
  let tls_config = Lazy.force _tls_config in
  let host =
    Uri.host uri
