(** Shared port-forward management used by [sun up] and [sun dev up/down/status].

    Implementation is extracted from [cmd_up.ml] and [cmd_dev.ml], with the
    absolute-path [state_dir] logic from [cmd_dev.ml] chosen as the canonical
    version. *)

(* ── State directory ──────────────────────────────────────────────────────── *)

let state_dir =
  (* Use an absolute path so all sun up/down/dev invocations share state
     regardless of cwd.  A relative ".sun" would mean cross-directory calls
     couldn't clean up port-forwards started from a different directory. *)
...
(* Read the cmdline of [pid] using /proc first, falling back to ps. *)
...
(** Parse -n <ns> and svc/<svc> from a kubectl port-forward argv list. *)
...
(** Extract the first "pid=<digits>" occurrence in [s]. *)
...
(** Find the PID that currently owns [local_port] using [ss]. *)
let pid_owning_port local_port =
  let tmp = Filename.temp_file "sun-ss-" ".tmp" in
  ignore (Sys.command
    (Printf.sprintf "ss -tlnp 'sport = :%d' > %s 2>/dev/null"
       local_port (Filename.quote tmp)));
  let ic = open_in tmp in
  let content = In_channel.input_all ic in
  close_in ic;
  (try Sys.remove tmp with _ -> ());
...
let start ~name ~namespace ~target ~local_port ~remote_port =
  ensure_state_dir ();
...
let check_liveness ~name ~local_port =
  Unix.sleepf 0.2;
  let pf = pid_file name in
  let alive =
...
let detect_stale local_port target_namespace target_service =
