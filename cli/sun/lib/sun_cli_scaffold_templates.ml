(* Template strings for [sun new] scaffold commands.
   ...
   All tpl_* values are literal file contents; ws_* values are templates
   for the workspace demo; svc_*, worker_*, fn_* values are templates
   for generic primitives; event_ml is the generic event template.

   Templates use quoted string heredoc syntax and support
   Sun_cli_scaffold.subst-style {{key}} substitution. *)

(* ── Shared root file templates ──────────────────────────────────────────── *)
