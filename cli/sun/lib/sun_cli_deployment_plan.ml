...
(* ── Render field types (formerly Sun_cli_deployment_render) ─────────────── *)
...
let render_spec ?(image = "") ?(secret_backend = Sun_cli_manifest.Kubernetes_live) (s : service_spec) =
  let common = {
  ...
  let workload = match s.primitive with
    ...
    | Fn ->
      ...
      Render_fn { schedule }
  in
  render_spec ~image ~secret_backend { common; workload }
